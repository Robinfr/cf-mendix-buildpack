...
class TestMicrometerMetricRegistry(TestCase):
    def setUp(self) -> None:
        self.addCleanup(patch.stopall)
        patch(
            "buildpack.runtime_components.metrics.micrometer_metrics_enabled",
            return_value=True,
        ).start()
    ...
    def test_paidapps_metrics_registry(self):
        with patch.dict(os.environ, {"PROFILE": "some-random-mx-profile"}):
            ...
            self.assertEqual(result.get('Metrics.Registries'), metrics.PAIDAPPS_METRICS_REGISTRY)

    def test_freeapps_metrics_registry(self):
        with patch.dict(os.environ, {"PROFILE": "free"}):
            result = metrics.configure_influx_registry(Mock())
            self.assertEqual(result.get('Metrics.Registries'), metrics.FREEAPPS_METRICS_REGISTRY)
